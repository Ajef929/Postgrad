...
checkPackages <- c("brnn","earth","glmnet","pls","rpart","rpart.plot","Cubist","KRLS","nnet","mboost","plsRglm","mgcv","MASS","pre","deepnet","spls","kernlab")
a<-installed.packages()
packages<-a[,1] 


for (item in checkPackages){
  if (!is.element(item, packages)){
    install.packages(c(item))
...
##have all of the models with this initial preprocessing step until final model selection tuning (which may include polynomial and interaction e.t.c. for linear models)
#common_initial <- c("naomit","dow", "month","dummy","scale","center","nzv","zv") ##nzv, nv "indicate_na" dummy scale centre
glmnet_initial <- c("impute_knn", "month","scale","center", "dummy")
...
brnn_initial <- c("impute_knn","dow", "month","scale","center","pca","dummy") ##nzv, nv "indicate_na" dummy scale centre

rpart_initial <- c("impute_knn","dow", "month","scale","center") # <-- These are arbitrary starting values. Set these to your best recommendation
# maintenance point ---------------------------------------------------------------------------------------------------------------------------
# add further preprocessing choices for the new methods here


common_initial <- c("impute_knn","scale","center","pca","dummy")

rvmLinear_initial <- c("impute_knn","dummy","scale","center")

bam_initial <- c("impute_knn")

cubist_initial <- c("impute_knn","month","dummy")

deepn_initial <- c("impute_knn","scale","center","pca","dummy")

spls_initial <- c("impute_knn","interact","scale","center","pca","dummy")

svmLinear_initial <- c("impute_knn","poly","month","dow","scale","center","pca","dummy")

glmboost_initial <- c("impute_knn","interact","dateDecimal","month","dow","scale","center","pca","dummy")

plsRglm_initial <- c("impute_knn","dow","dateDecimal","interact","scale","center", "dummy")

bagEarth_initial <- c("impute_knn","dow","dateDecimal","month","interact","scale","center","pca", "dummy")




##all of the possible choices of models
modelChoices <- c("NULL","GLMnet","Rpart","PLS","cubist","brnn","bagEarth","krlsPoly","pcaNNet","glmboost","plsRglm","bam","rlm","pre","ppr","dnn","spls","svmLinear")
